...
class WalkThrough {
public:
    explicit WalkThrough(MV::Camera *camera);
    ...
    void add_keyframe(const MV::Frame &frame, bool quiet = false);
    //@}
    ...
    void set_scene(const std::vector<MV::Model *> &scene);
    ...
    void walk_to(const MV::vec3 &ground_point);
    //@}
    ...
    void generate_camera_path(const MV::Line3 &axis);
    //@}
    ...
    MV::KeyFrameInterpolator *interpolator() const { return kfi_; }

protected:
    void add_keyframe(const MV::vec3 &cam_pos, const MV::vec3 &view_dir);
    ...
    MV::vec3 character_head(const MV::vec3 &foot) const;

    // convert view direction to orientation
    MV::quat to_orientation(const MV::vec3 &view_dir) const;

protected:
    MV::Camera *camera_;
    MV::KeyFrameInterpolator *kfi_;
