...
namespace MV {
	...
	public: //---------------------------------------------------- circulator types
	    ...
	    public:
	        ...
	        {
				iterator_ = graph_->m_vconn[vertex_].edges_.begin();
				end_ = graph_->m_vconn[vertex_].edges_.end();
	        }
			...
			operator bool() const { return !graph_->m_vconn[vertex_].edges_.empty(); }

			/// return current vertex
			Vertex vertex() const { return vertex_; }

			// helper for C++11 range-based for-loops
			EdgeAroundVertexCirculator& begin() { iterator_ = graph_->m_vconn[vertex_].edges_.begin(); return *this; }
			// helper for C++11 range-based for-loops
	...
	public: //---------------------------------------------- low-level connectivity
		...
		Vertex vertex(Edge e, unsigned int i) const
		{
			assert(i<=1);
			if (i == 0)
			    return m_econn[e].source_;
			else
                return m_econn[e].target_;
		}

        /// returns the starting vertex of an edge, which is equal to vertex(e, 0).
        Vertex source(Edge e) const { return m_econn[e].source_; }

        /// returns the ending vertex of an edge, which is equal to vertex(e, 1).
        Vertex target(Edge e) const { return m_econn[e].target_; }
		//@}
	...
	private: //------------------------------------------------------- private data
		...
		EdgeProperty<EdgeConnectivity>          m_econn;
