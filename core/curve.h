...
namespace MV {
    ...
    namespace curve {
         ...
         *     curve->set_impostor_type(MV::LinesDrawable::CYLINDER);
     ...
     * Example:
     *  \code
     *      auto cv = new MV::curve::Bezier<vec3>;
     *      curve->set_steps(num);
     ...
     * Example:
     *  \code
     *      auto cv = new MV::curve::BSpline<vec3>;
     *      curve->set_steps(num);
     *      for (const auto& p : points)
     *      cv->add_way_point(p);
     *      for (int i = 0; i < cv->node_count(); ++i)
     ...
     * Example:
     *  \code
     *      auto cv = new MV::curve::CatmullRom<vec3>;
     *      curve->set_steps(num);
     *      for (const auto& p : points)
     *      cv->add_way_point(p);
     *      for (int i = 0; i < cv->node_count(); ++i)
     *      std::cout << cv->node(i) << std::endl;
     *  \endcode
     * \class CatmullRom MV/core/curve.h
     */
    template<typename Point_t>
