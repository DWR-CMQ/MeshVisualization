...
namespace MV {
    ...
    PolyMesh::PolyMesh()
    {
        ...
        m_hconn    = add_halfface_property<HalfFaceConnectivity>("h:connectivity");
    ...
    PolyMesh& PolyMesh::operator=(const PolyMesh& rhs)
    {
        if (this != &rhs)
        {
            ...
            m_hconn    = halfface_property<HalfFaceConnectivity>("h:connectivity");
    ...
    PolyMesh& PolyMesh::assign(const PolyMesh& rhs)
    {
        if (this != &rhs)
        {
            ...
            m_vprops.resize(rhs.n_vertices());
            cprops_.resize(rhs.n_cells());
            m_eprops.resize(rhs.n_edges());
    ...
    void PolyMesh::clear()
    {
        ...
        m_vprops.resize_property_array(2);   // "v:connectivity", "v:point"
        m_eprops.resize_property_array(1);   // "e:connectivity"
    ...
    PolyMesh::HalfFace PolyMesh::add_face(const std::vector<Vertex>& vertices) {
        ...
        if (!h.is_valid()) {
            h = new_face();
            auto oh = opposite(h);
            m_hconn[h].vertices_ = vertices;
            m_hconn[oh].vertices_ = std::vector<Vertex>(vertices.rbegin(), vertices.rend());
            ...
            for (std::size_t i=0; i<vertices.size(); ++i) {
                ...
                m_econn[e].halffaces_.insert(h);
                m_econn[e].halffaces_.insert(oh);
                m_hconn[h].edges_.insert(e);
                m_hconn[oh].edges_.insert(e);

                m_vconn[s].halffaces_.insert(h);
                m_vconn[s].halffaces_.insert(oh);
            }
        }
    ...
    vec3 PolyMesh::compute_face_normal(Face f) const
    {
        ...
        if (vts.size() == 3) { // face is a triangle
            const vec3& p0 = m_vpoint[vts[0]];
            const vec3& p1 = m_vpoint[vts[1]];
            const vec3& p2 = m_vpoint[vts[2]];
            ...
            for (std::size_t id = 0; id < vts.size(); ++id) {
                ...
                vec3 p0 = m_vpoint[vts[id_prev]];
                vec3 p1 = m_vpoint[vts[id]];
                vec3 p2 = m_vpoint[vts[id_next]];
