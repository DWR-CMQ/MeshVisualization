...
namespace MV {
    ...
    SurfaceMesh::SurfaceMesh()
    {
        ...
        m_hconn    = add_halfedge_property<HalfedgeConnectivity>("h:connectivity");
        m_fconn    = add_face_property<FaceConnectivity>("f:connectivity");
        ...
        m_fdeleted = add_face_property<bool>("f:deleted", false);

        m_mprops.push_back();

        m_uideletedvertices = m_uideletededges = m_deleted_faces = 0;
        m_bgarbage = false;
    }
    ...
    SurfaceMesh& SurfaceMesh::operator=(const SurfaceMesh& rhs)
    {
        if (this != &rhs)
        {
            ...
            m_hconn    = halfedge_property<HalfedgeConnectivity>("h:connectivity");
            m_fconn    = face_property<FaceConnectivity>("f:connectivity");
            m_vdeleted = vertex_property<bool>("v:deleted");
            m_edeleted = edge_property<bool>("e:deleted");
            m_fdeleted = face_property<bool>("f:deleted");
            m_vpoint   = vertex_property<vec3>("v:point");

            // normals might be there, therefore use get_property
            m_vnormal  = get_vertex_property<vec3>("v:normal");
            m_fnormal  = get_face_property<vec3>("f:normal");

            // how many elements are deleted?
            m_uideletedvertices = rhs.m_uideletedvertices;
            m_uideletededges    = rhs.m_uideletededges;
            m_deleted_faces    = rhs.m_deleted_faces;
    ...
    SurfaceMesh &SurfaceMesh::join(const SurfaceMesh &other) {
        ...
        m_eprops.transfer(other.m_eprops);

        // translate halfedge index in vertex -> halfedge
        for (unsigned int i = nv; i < nv + other.vertices_size(); i++) {
            Vertex vi(static_cast<int>(i));
            if (m_vconn[vi].halfedge_ != Halfedge()) {
                m_vconn[vi].halfedge_ = Halfedge(m_vconn[vi].halfedge_.idx() + static_cast<int>(nh));
            }
        }
        // translate halfedge index in face -> halfedge
        for (unsigned int i = nf; i < nf + other.faces_size(); i++) {
            Face fi(static_cast<int>(i));
            if (m_fconn[fi].halfedge_ != Halfedge()) {
                m_fconn[fi].halfedge_ = Halfedge(m_fconn[fi].halfedge_.idx() + static_cast<int>(nh));
            }
        }
        ...
        for (unsigned int i = nh; i < nh + other.halfedges_size(); i++) {
            ...
            if (m_hconn[hi].face_ != Face()) {
                m_hconn[hi].face_ = Face(m_hconn[hi].face_.idx() + static_cast<int>(nf));
            }
            if (m_hconn[hi].vertex_ != Vertex()) {
                m_hconn[hi].vertex_ = Vertex(m_hconn[hi].vertex_.idx() + static_cast<int>(nv));
            }
            if (m_hconn[hi].next_ != Halfedge()) {
                m_hconn[hi].next_ = Halfedge(m_hconn[hi].next_.idx() + static_cast<int>(nh));
            }
            if (m_hconn[hi].prev_ != Halfedge()) {
                m_hconn[hi].prev_ = Halfedge(m_hconn[hi].prev_.idx() + static_cast<int>(nh));
            }
        }
        ...
        m_uideletededges += other.m_uideletededges;
        m_deleted_faces += other.m_deleted_faces;
        return *this;
    }
    ...
    void SurfaceMesh::clear()
    {
        ...
        m_vprops.shrink_to_fit(); // remove unused memory from vectors
        ...
        m_eprops.resize_property_array(1);   // "e:deleted"
        fprops_.resize_property_array(2);   // "f:connectivity", "f:deleted"
        m_mprops.clear();
        m_mprops.resize(1);

        // update/invalidate the normal properties
        m_vnormal  = VertexProperty<vec3>();
        m_fnormal  = FaceProperty<vec3>();
    }
        ...
        for (i=0, ii=1; i<n; ++i, ++ii, ii%=n)
        {
            if ( !is_border(vertices[i]) )
            {
                ...
                if (show) {
                    ...
                    LOG(ERROR) << "\t\t" << vertices[i] << ": " << m_vpoint[vertices[i]];
                    LOG(ERROR) << "\tvertices of the face: ";
                    for (auto v : vertices) LOG(ERROR) << "\t\t" << v << ": " << m_vpoint[v];
                    show = false;
                }
#endif
                return Face();
            ...
            if (!is_new[i] && !is_border(halfedges[i]))
            {
                ...
                if (show) {
                    LOG(ERROR) << "\tvertex coordinates: ";
                    LOG(ERROR) << "\t\t" << vertices[i] << ": " << m_vpoint[vertices[i]];
                    LOG(ERROR) << "\t\t" << vertices[ii] << ": " << m_vpoint[vertices[ii]];
                    LOG(ERROR) << "\tvertices of the face: ";
                    for (auto v : vertices) LOG(ERROR) << "\t\t" << v << ": " << m_vpoint[v];
                    show = false;
                }
    ...
    {
        if (!m_fnormal)
            m_fnormal = face_property<vec3>("f:normal");
        ...
        for (fit=faces_begin(); fit!=fend; ++fit) {
            if (is_degenerate(*fit)) {
                ++num_degenerate;
                m_fnormal[*fit] = vec3(0, 0, 1);
            } else
                m_fnormal[*fit] = compute_face_normal(*fit);
        }
    ...
    vec3 SurfaceMesh::compute_face_normal(Face f) const
    {
        ...
        vec3 p0 = m_vpoint[target(h)];
        h = next(h);
        vec3 p1 = m_vpoint[target(h)];
        h = next(h);
        vec3 p2 = m_vpoint[target(h)];
    ...
    {
        if (!m_vnormal)
            m_vnormal = vertex_property<vec3>("v:normal");
        ...
        for (VertexIterator vit=vertices_begin(), vend = vertices_end(); vit!=vend; ++vit)
            m_vnormal[*vit] = compute_vertex_normal(*vit);
    }
    ...
    vec3 SurfaceMesh::compute_vertex_normal(Vertex v) const
    {
        ...
        if (h.is_valid())
        {
            ...
            do
            {
                if (!is_border(h))
                {
                    p1 = m_vpoint[target(h)];
                    p1 -= p0;

                    p2 = m_vpoint[source(prev(h))];
                    p2 -= p0;
    ...
    float SurfaceMesh::edge_length(Halfedge h) const {
        return norm(m_vpoint[source(h)] - m_vpoint[target(h)]);
    }
    ...
    void SurfaceMesh::stitch(Halfedge h0, Halfedge h1) {
        ...
        const vec3& p_org0 = geom::barycenter(m_vpoint[org0], m_vpoint[dest1]);
        const vec3& p_org1 = geom::barycenter(m_vpoint[dest0], m_vpoint[org1]);

        Halfedge new_h0 = new_edge(org1, org0);
        Halfedge new_h1 = opposite(new_h0);
        m_vpoint[org0] = p_org0;
        m_vpoint[org1] = p_org1;
    ...
    void SurfaceMesh::remove_edge(Halfedge h)
    {
        ...
        // delete stuff
        if (!m_vdeleted) m_vdeleted = vertex_property<bool>("v:deleted", false);
        if (!m_edeleted) m_edeleted = edge_property<bool>("e:deleted", false);
        m_vdeleted[vo]      = true; ++m_uideletedvertices;
        m_edeleted[edge(h)] = true; ++m_uideletededges;
        m_bgarbage = true;
    }
    ...
    void SurfaceMesh::remove_loop(Halfedge h)
    {
        ...
        // delete stuff
        if (!m_edeleted) m_edeleted = edge_property<bool>("e:deleted", false);
        if (!m_fdeleted) m_fdeleted = face_property<bool>("f:deleted", false);
        if (fh.is_valid()) { m_fdeleted[fh] = true; ++m_deleted_faces; }
        m_edeleted[edge(h0)] = true; ++m_uideletededges;
        m_bgarbage = true;
    }
    ...
    void SurfaceMesh::collect_garbage()
    {
        ...
        m_vprops.resize(nV); m_vprops.shrink_to_fit();
        hprops_.resize(nH); hprops_.shrink_to_fit();
        m_eprops.resize(nE); m_eprops.shrink_to_fit();
