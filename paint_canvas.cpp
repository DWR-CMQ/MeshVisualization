...
#include "renderer/manipulated_camera_frame.h"
#include "renderer/key_frame_interpolator.h"
#include "renderer/ambient_occlusion.h"
#include "renderer/soft_shadow.h"
#include "renderer/dual_depth_peeling.h"
#include "renderer/eye_dome_lighting.h"
#include "renderer/read_pixel.h"
#include "renderer/opengl_util.h"
#include "renderer/opengl_error.h"
...
PaintCanvas::PaintCanvas(MeshWindow* window)
        ...
        , ssao_(nullptr)
{
    ...
    MV::connect(&camera_->frame_modified, this, static_cast<void (PaintCanvas::*)(void)>(&PaintCanvas::update));
...
void PaintCanvas::mouseReleaseEvent(QMouseEvent *e) 
{
    ...
    //}
    //else 
    if (pressed_button_ == Qt::LeftButton && e->modifiers() == Qt::ControlModifier) { // ZOOM_ON_REGION
...
void PaintCanvas::mouseMoveEvent(QMouseEvent *e) 
{
    ...
    if (show_primitive_id_under_mouse_ || show_primitive_property_under_mouse_)
    { // then need to pick primitives
        picked_face_index_ = -1;
        picked_vertex_index_ = -1;
        ...
        if (show_primitive_property_under_mouse_) 
        {
            if (picked_vertex_index_ >= 0) { // picked point cloud vertex
                ...
                if (drawable->coloring_method() == MV::State::SCALAR_FIELD) {
                ...
                if (drawable->coloring_method() == MV::State::SCALAR_FIELD && drawable->property_location() == MV::State::FACE) {
                    const std::string& name = drawable->property_name();
    ...
    {
        bool found = false;
        const MV::vec3 p = pointUnderPixel(e->pos(), found);
        QString coords= "XYZ = [-, -, -]";
        if (found)
        {
...
void PaintCanvas::setCurrentModel(MV::Model *m)
{
...
void PaintCanvas::fitScreen(const MV::Model *model) {
...
void PaintCanvas::invertSelection() {
    ...
    if (dynamic_cast<SurfaceMesh*>(currentModel())) {
        ...
        if (d->coloring_method() != MV::State::SCALAR_FIELD || d->property_name() != "f:select") {
    ...
    //{
        ...
        //if (d->coloring_method() != MV::State::SCALAR_FIELD || d->property_name() != "v:select")
        //{
...
void PaintCanvas::restoreState(std::istream& input) {
    ...
    MV::disconnect_all(&camera_->frame_modified);
...
void PaintCanvas::draw() {
    ...
    for (const auto m : models_)
    {
        ...
        std::size_t count = 0;
        std::cout << " m->renderer()->lines_drawables(): " << m->renderer()->lines_drawables().size() << std::endl;
        std::cout << " m->renderer()->points_drawables(): " << m->renderer()->points_drawables().size() << std::endl;
        std::cout << " m->renderer()->triangles_drawables(): " << m->renderer()->triangles_drawables().size() << std::endl;
